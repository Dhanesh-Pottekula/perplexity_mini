...
class Embedding:
    ...
    async def upsert_embeddings_topics_qdrant(self, texts: List[StorageAgentPydanticModels.TopicExtractionList]) -> None:
        """Generate embeddings for texts and upsert them to Qdrant collection"""
        # Generate embeddings
        try:
            embeddings = self.model.encode(texts, convert_to_numpy=True).tolist()
            ...
            for i, (text, embedding) in enumerate(zip(texts, embeddings)):
                ...
                point = PointStruct(
                    ...
                    vector=embedding,
                    payload={
                        PayloadKeys.TOPIC_ID: text.topic_name,
                        PayloadKeys.TEXT: text,
                        PayloadKeys.INDEX: i,
                    },
                )
            ...
            # Upsert to Qdrant
            self.client.upsert(collection_name=CollectionNames.TOPICS, points=points)
